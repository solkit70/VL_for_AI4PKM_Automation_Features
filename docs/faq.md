...
A: Follow the [Getting Started](index.html#getting-started) guide on the homepage. Key steps:
1. Clone the repo
2. Open in Obsidian
...
A: Edit `cron.json` in the root directory. See [Workflow Automation](workflows.html#workflow-automation) for examples.
...
See [Workflows](workflows.html) for detailed explanations.
...
A: Yes! All prompts are in `_Settings_/Prompts/` as markdown files. Edit them to suit your needs. Follow the standard [Prompt Template](prompts.html#standardized-template-structure) structure.
...
- Check the [Guidelines](guidelines.html) for system architecture
- Review [Prompts](prompts.html) for detailed prompt documentation
- See [Workflows](workflows.html) for workflow specifics
- Read the [CLI Tool](cli_tool.html) for command reference
