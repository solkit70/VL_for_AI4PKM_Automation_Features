"""Integration test for KTG/KTP/KTE workflow - Limitless "Hey PKM" scenario.

This test simulates the complete end-to-end workflow:
1. Starts ai4pkm watchdog in test mode
2. Creates test Limitless conversation file with "hey pkm" trigger
3. Monitors: LimitlessFileHandler → Request JSON → KTG → KTP → KTE
4. Validates completion and cleanup

Test validates:
- "hey pkm" detection in Limitless conversations
- JSON request generation with metadata
- Task creation with proper status
- TBD task polling detection
...
class KTGKTPLimitlessIntegrationTest:
    """Integration test for KTG/KTP/KTE Limitless workflow."""
    ...
    def __init__(self):
        ...
        self.test_limitless_name = f"2025-10-17-test-{self.test_timestamp}.md"
        self.test_limitless_path = os.path.join(
            self.workspace_path,
            "Ingest",
            "Limitless",
            self.test_limitless_name
        )

        # Paths to monitor
        self.requests_dir = os.path.join(self.workspace_path, "AI", "Tasks", "Requests", "Limitless")
    ...
    def start_watchdog(self):
        ...
        try:
            ...
            watchdog_log = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'watchdog_limitless_output.log')
    ...
    def create_test_limitless_file(self):
        """Create test Limitless conversation file from template."""
        self.logger.info("=" * 80)
        self.logger.info("STAGE 1: Creating test Limitless conversation file")
        self.logger.info("=" * 80)
        ...
        try:
            ...
            # Ensure Limitless directory exists
            limitless_dir = os.path.join(self.workspace_path, "Ingest", "Limitless")
            os.makedirs(limitless_dir, exist_ok=True)

            # Write test file
            with open(self.test_limitless_path, 'w', encoding='utf-8') as f:
                f.write(content)

            self.created_files.append(self.test_limitless_path)
            self.logger.info(f"  ✅ Created: {self.test_limitless_path}")

            self.test_results['stages']['limitless_creation'] = {
                'success': True,
                'timestamp': datetime.now(),
                'file_path': self.test_limitless_path
            }

        except Exception as e:
            self.logger.error(f"  ❌ Failed to create Limitless file: {e}")
            self.test_results['stages']['limitless_creation'] = {
                'success': False,
                'timestamp': datetime.now(),
                'error': str(e)
    ...
    def wait_for_request_generation(self, timeout=60):
        """Wait for Limitless request JSON to be generated."""
        self.logger.info("=" * 80)
        self.logger.info("STAGE 2: Waiting for Limitless request generation")
        self.logger.info("=" * 80)
            ...
            try:
                ...
                if os.path.exists(self.requests_dir):
                    ...
                    if files:
                        ...
                        self.logger.info(f"  Total requests: {request_data.get('metadata', {}).get('total_requests', 0)}")

                        # Validate request structure
                        if 'metadata' in request_data and 'requests' in request_data:
                            requests = request_data['requests']
                            if requests:
                                first_req = requests[0]
                                self.logger.info(f"  Has preference: {first_req.get('has_preference', False)}")
    ...
    def wait_for_task_creation(self, timeout=120):
        """Wait for KTG to create task file from Limitless request."""
        self.logger.info("=" * 80)
        ...
        start_time = time.time()
        task_file = None
        expected_prefix = "2025-10-17"  # Tasks created today

        # Remember start timestamp to filter only new files
        test_start_timestamp = self.test_results['start_time'].timestamp()
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if os.path.exists(self.tasks_dir):
                    ...
                    # Filter: not in created_files AND modified after test started
                    new_files = []
                    for f in files:
                        full_path = os.path.join(self.tasks_dir, f)
                        if full_path not in self.created_files:
                            # Check if file was created/modified after test started
                            file_mtime = os.path.getmtime(full_path)
                            if file_mtime >= test_start_timestamp:
                                new_files.append(f)

                    if new_files:
                        # Found task file - take the most recent one
                        task_file = os.path.join(self.tasks_dir, max(new_files,
                                               key=lambda f: os.path.getmtime(os.path.join(self.tasks_dir, f))))
    ...
    def generate_report(self):
        ...
        report_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            f'KTG_KTP_Limitless_Integration_Test_Results_{self.test_timestamp}.md'
        )

        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("# KTG/KTP/KTE Integration Test Results - Limitless Scenario\n\n")
    ...
    def run(self):
        ...
        try:
            ...
            # Create test Limitless file
            self.create_test_limitless_file()
